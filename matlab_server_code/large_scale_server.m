...
for num_net = 6
%test = [17,91,126];
%test = [79,84,91,98,105,112,119,126,133,140];
test = 35;
...
%remake
test_graph = net(num_net);
%parfor seed_chose = 1:length(test)
for seed_chose = 1:length(test)    
[G, EdgeTable, NodeTable] = read_graph(test_graph);
a = 1;
b = 5;
...
rep ={};
base = 0;
seed_chosen = test(seed_chose);
seed = seed_verify_server(seed_chosen,test_graph);
rng(seed);
...
%flag = [500 250 150];
coefficient_alpha = zeros(size(NodeTable.Name,1),num_slices);
coefficient_beta = zeros(size(EdgeTable.EndNodes,1),num_slices);
for s = 1:num_slices
...
%save(['final_real_arrival_1s_50ms_5ms_slices_final_',convertStringsToChars(test_graph),'_',num2str(seed),'_complexity_delay1e4_iter200_contolerance1-2_inter_',num2str(num_iteration),'_test_greedy_',num2str(greedy_num),'.mat'],'x','fval','res','f_sta','f_prop_50','f_prop_75','f_prop_100','f_opt','prob_opt','prob_prop_100','prob_prop_75','prob_prop_50','x_range_min','x_range_prop_50','x_range_prop_75','x_range_prop_100','prob_fail','rep','t_app','tEnd');
parsave(['final_real_placement_1s_50ms_5ms_slices_final_',convertStringsToChars(test_graph),'_',num2str(seed),'_complexity_delay1e4_iter200_contolerance1-2_inter_',num2str(num_iteration),'_test_greedy_',num2str(greedy_num),'.mat'],x,fval,res,f_sta,f_prop_50,f_prop_75,f_prop_100,f_opt,prob_opt,prob_prop_100,prob_prop_75,prob_prop_50,x_range_min,x_range_prop_50,x_range_prop_75,x_range_prop_100,prob_fail,rep,t_app,tEnd);
