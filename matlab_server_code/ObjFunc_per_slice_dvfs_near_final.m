function f_final = ObjFunc_per_slice_dvfs_near_final(x,traffic_path,coefficient_alpha,NodeTable,EdgeTable,num_slices,r_max)
...
for i = 1:size(NodeTable.Name,1)
    for s = 1:num_slices
        if  r_max(i+(s-1)*(size(NodeTable.Name,1)+size(EdgeTable.EndNodes,1))) ~= 0
                   f(s,i) = x(i+(s-1)*(size(NodeTable.Name,1)+size(EdgeTable.EndNodes,1)))/r_max(i+(s-1)*(size(NodeTable.Name,1)+size(EdgeTable.EndNodes,1))) *rho(s,i)*11.4456*(x(i+(s-1)*(size(NodeTable.Name,1)+size(EdgeTable.EndNodes,1)))/3e3) - 2.6805;
        end
    end
end
