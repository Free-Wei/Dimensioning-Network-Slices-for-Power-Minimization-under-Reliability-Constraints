...
test = [84,91,98,105,112,119,126,133,140];
%test = 78;
%test = [7,14,21,28,35,42,49,56,63,70];
%test = 49;
%test = [98,105,112,119,126,133,140];
%test = [56,63,70,77,84,91,98,105,112,119,126,133,140];
...
for s = 1:num_slices
    ...
    list_each_slice{s} = listForSample;
end
w_dup = 0.1*(num_iteration-1);
%w_dup = 0.1;
for s = 1:num_slices
    probability_s = [];
    for i = 2 : num_components_slice(s)
        if randsample([0,1],1,true, [1-w_dup w_dup]) == 1
            %rng(seed);
            second_rep = randsample(list_each_slice{s}, 1);
            while ismember(second_rep ,replicas_destinations{s}{i})
                %rng(seed);
                second_rep = randsample(list_each_slice{s},1);
            end
            replicas_destinations{s}{i} = [replicas_destinations{s}{i} second_rep];
        end
        len_components(s) = length(replicas_destinations{s}{i});
        for l = 1:size(replicas_destinations{s}{i},2)
            val = find(list_each_slice{s}== replicas_destinations{s}{i}(l));
            list_each_slice{s}(val) = [];
        end
...
opts = optimoptions(opts,'MaxFunctionEvaluations',9e6,'MaxIterations',500, 'StepTolerance',1e-10,'ConstraintTolerance',1e-6,'PlotFcn',{@optimplotx,@optimplotfval,@optimplotfirstorderopt}); % Recommended
...
save(['final_real_duplicated_prob_1s_50ms_5ms_slices_final_Twaren.graphml_',num2str(seed),'_complexity_delay1e4_iter200_contolerance1-2_inter_',num2str(num_iteration),'_test_greedy_',num2str(greedy_num),'.mat'],'x','fval','res','f_sta','f_prop_50','f_prop_75','f_prop_100','f_opt','prob_opt','prob_prop_100','prob_prop_75','prob_prop_50','x_range_min','x_range_prop_50','x_range_prop_75','x_range_prop_100','prob_fail','rep','t_app','tEnd');
